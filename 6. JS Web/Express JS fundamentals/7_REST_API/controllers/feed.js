...
module.exports = {
  getPosts: (req, res) => {//когато стигнем дотук вече няма да рендерираме за клиента html-a, а ще му връщаме ресурс под формата на json, който ресурс клиета ще си обработва
    // Retrieve all posts in JSON format
    Post.find()//вадим от базата всички постове (в случая)
      .then((posts) => {
        res
          .status(200)//закачаме задължително статуса
          .json({ message: 'Fetched posts successfully.', posts });//пишем някакво съобщение и изпращаме това, което сме взели от базата и метода json го прави в json формат
      })
      .catch((error) => {
        if (!error.statusCode) {
